...
@[simp] lemma lt [L.Mem] {t u : Semiterm L ξ n} : Hierarchy Γ s “!!t ∈ !!u” := by
  simp [Semiformula.Operator.operator, Matrix.fun_eq_vec_two, Semiformula.Operator.Mem.sentence_eq]
...
@[simp] lemma rew_iff {ω : Rew L ξ₁ n₁ ξ₂ n₂} {φ : Semiformula L ξ₁ n₁} :
  ...
  · generalize eq : ω ▹ φ = ψ
    ...
    case or ihp ihq =>
      ...
      simpa using ⟨ihp rfl, ihq rfl⟩
    case ball pos _ ih =>
      simp only [Rew.eq_mem_iff, Rew.q_eq_zero_iff, Matrix.vecCons_empty_eq_singleton,
...
lemma sigma₁_induction {P : (n : ℕ) → Semiformula ℒₛₑₜ ξ n → Prop}
    (hVerum : ∀ n, P n ⊤)
    (hFalsum : ∀ n, P n ⊥)
  ...
  |  Hierarchy.rel _ _ Language.Mem.mem v => by simpa [←Matrix.fun_eq_vec_two] using hMem _ (v 0) (v 1)
  | Hierarchy.nrel _ _ Language.Mem.mem v => by simpa [←Matrix.fun_eq_vec_two] using hNMem _ (v 0) (v 1)
  |                 Hierarchy.and hp hq =>
    hAnd _ _ _ hp hq
      (sigma₁_induction hVerum hFalsum hEQ hNEQ hMem hNMem hAnd hOr hBall hExs _ _ hp)
      (sigma₁_induction hVerum hFalsum hEQ hNEQ hMem hNMem hAnd hOr hBall hExs _ _ hq)
  |                  Hierarchy.or hp hq =>
    hOr _ _ _ hp hq
      (sigma₁_induction hVerum hFalsum hEQ hNEQ hMem hNMem hAnd hOr hBall hExs _ _ hp)
      (sigma₁_induction hVerum hFalsum hEQ hNEQ hMem hNMem hAnd hOr hBall hExs _ _ hq)
  |                Hierarchy.ball pt hp => by
    ...
    exact hBall _ t _ hp (sigma₁_induction hVerum hFalsum hEQ hNEQ hMem hNMem hAnd hOr hBall hExs _ _ hp)
  |                 Hierarchy.bexs pt hp => by
    apply hExs
    · simp [hp]
    · rcases Rew.positive_iff.mp pt with ⟨t, rfl⟩
      apply hAnd _ _ _ (by simp) hp (by simpa [Semiformula.Operator.lt_def] using hMem _ _ _)
        (sigma₁_induction hVerum hFalsum hEQ hNEQ hMem hNMem hAnd hOr hBall hExs _ _ hp)
  |         Hierarchy.sigma (φ := φ) hp =>
    have : Hierarchy 𝚺 1 φ := hp.accum _
    hExs _ _ this (sigma₁_induction hVerum hFalsum hEQ hNEQ hMem hNMem hAnd hOr hBall hExs _ _ this)
  |                    Hierarchy.exs hp =>
    hExs _ _ hp (sigma₁_induction hVerum hFalsum hEQ hNEQ hMem hNMem hAnd hOr hBall hExs _ _ hp)

lemma sigma₁_induction' {n φ} (hp : Hierarchy 𝚺 1 φ)
    {P : (n : ℕ) → Semiformula ℒₛₑₜ ξ n → Prop}
    (hVerum : ∀ n, P n ⊤)
    (hFalsum : ∀ n, P n ⊥)
    (hEQ : ∀ n t₁ t₂, P n (.rel Language.Eq.eq ![t₁, t₂]))
    (hNEQ : ∀ n t₁ t₂, P n (.nrel Language.Eq.eq ![t₁, t₂]))
